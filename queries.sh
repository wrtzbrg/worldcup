...
echo "$( $PSQL "select DISTINCT(name) from games left join teams as winner on games.winner_id = winner.team_id order by name;" )"

echo -e "\nYear and team name of all the champions:"
echo "$( $PSQL "select year, name from games left join teams as winner on games.winner_id = winner.team_id WHERE round='Final' order by year asc" )"
